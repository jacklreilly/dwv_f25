...
5. where “protected lands” is on the Y axis and GDP per capita is on the X axis.  Then, overlay the plot with a line representing the slope.
5. Regress “protected land” on total GDP.

### Part 3. National Election Studies
1. Using `nes2000subset_age.csv`, regress partisanship on age.
6. Plot the resulting regression line, and properly label all axes. (Hint: don't forget to rescale the X axis!)
7. Color the line! Label the line next to itself with "OLS Line"
...
3. Either:
    ...
    - Share the repo with me (I'm `@jacklreilly` on GitHub).
4. On Blackboard, under assginment 4, upload a link to the repo so that I can click straight through to it.